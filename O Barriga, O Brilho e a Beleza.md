...
2. **Pokemon**: catálogo geral do torneio, contendo as espécies existentes (alimentado pela PokeAPI). Registra-se o **id da espécie, nome da espécie, tipo base, tipo secundário (quando houver) e para qual espécie ela evolui** (quando aplicável).

3. **Golpe:** catálogo geral de habilidades que os Pokémon podem utilizar nos confrontos (alimentado pela PokeAPI). Cada golpe possui **id do golpe, nome, tipo elemental, poder de ataque, precisão e pontos de poder (PP) máximos.**

4. **Time_Treinador:** tabela associativa que registra quais espécies de Pokémon cada treinador inscreveu no torneio. Possui como atributos **Treinador_id_treinador** (Chave Estrangeira) e **Pokemon_id_especie** (Chave Estrangeira), formando juntos a chave primária composta da tabela.
...
6. **Batalha:** histórico oficial das partidas. Cada registro armazena um **ID único, a data do confronto, a fase do torneio, os IDs dos dois treinadores envolvidos (Treinador 1 e Treinador 2) e o ID do treinador que saiu vitorioso.**
